...
class TestSanitizeInputCleanInputs:
    """Tests that legitimate Intugle Flow questions pass sanitization."""

    def test_should_pass_normal_langflow_question(self):
        """Normal Intugle Flow question should pass."""
        result = sanitize_input("How do I create a component in Intugle Flow?")
    ...
    def test_should_pass_system_in_legitimate_context(self):
        ...
        result = sanitize_input("How do I configure system settings in Intugle Flow?")
        assert result.is_safe is True

    def test_should_pass_portuguese_question(self):
        """Portuguese Intugle Flow question should pass."""
        result = sanitize_input("Como criar um fluxo no Intugle Flow?")
        assert result.is_safe is True
...
class TestRefusalMessage:
    ...
    def test_refusal_message_mentions_intugle_flow(self):
        """Refusal message should mention Intugle Flow to redirect the user."""
        assert "Intugle Flow" in REFUSAL_MESSAGE
