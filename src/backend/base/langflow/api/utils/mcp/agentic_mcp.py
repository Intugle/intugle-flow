"""Utilities for auto-configuring the Intugle Flow Agentic MCP server."""
...
async def auto_configure_agentic_mcp_server(session: AsyncSession) -> None:
    ...
    This function adds the intugle-flow-agentic MCP server to each user's MCP
    ...
    await logger.ainfo("Auto-configuring Intugle Flow Agentic MCP server for all users...")
    ...
    try:
        ...
        server_config = {
            ...
            "metadata": {
                "description": "Intugle Flow Agentic MCP server providing tools for flow/component operations, "
...
async def remove_agentic_mcp_server(session: AsyncSession) -> None:
    """Remove the Intugle Flow Agentic MCP server from all users.

    This function removes the intugle-flow-agentic MCP server from each user's MCP
    configuration. Used when agentic experience is disabled.

    Args:
        session: Database session for querying users.
    """
    await logger.ainfo("Removing Intugle Flow Agentic MCP server from all users...")
